...
def load_previous_state(df, df_test, state, model_folder, num_epochs, config, rank=0):    
    ...
    train_loss = torch.zeros([num_epochs, num_batches_train])
    train_acc = torch.zeros([num_epochs, num_batches_train])
    val_loss = torch.zeros([num_epochs, num_batches_test])
    val_acc = torch.zeros([num_epochs, num_batches_test])    
...
def forward_val(df_train, config, bert_model, classification_model, optimizer, scheduler, criterion, epoch_ind, total_time, 
    processed_batches, device, num_epochs, rank, MASK_id, PAD_id, CLS_id, UNK_id, vocab_size, vocab, Fake_position=None, num_obs=10):
    if rank==0:
        print("forward train")    
    max_position_embeddings = config["model"]["max_position_embeddings"]
    ...
    perform  = torch.zeros(2, num_batches)-1.0
    bert_model.eval()
    ...
    for ex, ex_pos, x_resp, x_pos, y_resp, y_pos, l, n_y in train_iter:
        ti = time.time()
        x = torch.repeat_interleave(ex,torch.sum(y_pos,1), dim=0).tolist()
        ...
        acc = (y_hat == targets).sum().item()/len(targets)
        #print(len(y_hat.tolist())), print(len(x)), print(len(y)), print(len(y_r)), print(len(y_hat)), print(len(classification_pred.tolist()))
        y_hat = y_hat.tolist()
        X = X + x
        Y = Y + y
        Y_resp = Y_resp + y_r
        ...
        #
        if rank == 0 and batches_count%10==0:
    ...
    df = pd.DataFrame(list(zip(X, Y, Y_resp, Y_pred, Y_score)), columns =['X', 'Y', 'Y_resp', 'Y_pred', 'Y_score'])
    print(df.shape)
    return df
...
def main_process(model_folder, config, is_dist=False):
    ...
    df_val = _read_file_ab(config["data"]["val_ds"])
    ...
    df_test_f = forward_val(df_test, config, bert_model, classification_model, optimizer, scheduler, criterion, epoch_ind, total_time, 
        processed_batches, device, num_epochs, rank, MASK_id, PAD_id, CLS_id, UNK_id, vocab_size, vocab, Fake_position, config["val"]["observations_test"])
    df_test_f.to_csv("test_final.csv", index=False)
    df_val_f = forward_val(df_val, config, bert_model, classification_model, optimizer, scheduler, criterion, epoch_ind, total_time, 
        processed_batches, device, num_epochs, rank, MASK_id, PAD_id, CLS_id, UNK_id, vocab_size, vocab, Fake_position, config["val"]["observations_val"])
    df_val_f.to_csv("val_final.csv", index=False)
    df_train_f = forward_val(df_train, config, bert_model, classification_model, optimizer, scheduler, criterion, epoch_ind, total_time, 
        processed_batches, device, num_epochs, rank, MASK_id, PAD_id, CLS_id, UNK_id, vocab_size, vocab, Fake_position, config["val"]["observations_train"])
    df_train_f.to_csv("train_final.csv", index=False)    
